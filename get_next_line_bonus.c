...
/*   Updated: 2023/11/30 15:22:10 by rboutaik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	{
		buffer = ft_allocat(&str[fd], BUFFER_SIZE);
		if (!buffer)
			return (NULL);
		size = read(fd, buffer, BUFFER_SIZE);
		v = f(&str[fd], &buffer, size);
		if (v == 0)
			break ;
		else if (v == 1 || v == 2)
			return (NULL);
	...
	buffer = ft_getstr(str[fd]);
	if (!buffer)
		return (free(str[fd]), str[fd] = NULL, NULL);
	return (str[fd] = ft_getsec(str[fd]), buffer);
}
